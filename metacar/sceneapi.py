...
class SceneAPI:
    ...
    def set_hybrid_delta(
        self,
        dx: float,
        dy: float,
        dt: float | None = None,
    ):
        """以 HybridControl.Delta 方式发送控制"""
        ...
        if dt is not None:
            hybrid_control["deltaTime"] = dt

        sim_car_msg = SimCarMsgOutput(
            hybrid_control=hybrid_control,
        )
        self._model_socket.send(Code4(code=4, sim_car_msg=sim_car_msg), Code4)
        # Yield once after sending hybrid control so the peer can process the packet promptly.
        sleep(0)
