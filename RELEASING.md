...
本文档用于发布独立产品 `metacar-hybrid`。Python 导入名仍为 `metacar`，产品版本号不跟随上游 MetaCar。
...
1. 从最新 `main` 创建发布准备分支。
2. 更新 `metacar/__init__.py` 中的 `__version__`。
...
7. 在 GitHub 创建新 Release，标签必须严格等于 `v` 加包版本，例如 `v0.1.0a1`。
...
9. 在干净环境验证：
   ...
   verify-env/Scripts/python -c "import metacar; print(metacar.__version__)"
   ```
...
- 如果 GitHub Release 的标签与 `metacar.__version__` 不一致，工作流会在上传前失败。
