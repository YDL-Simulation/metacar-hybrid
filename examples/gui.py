...
def tk_process_func(
    ...
    PARKING_REGION_STYLE_MAP = {
        RegionType.NORMAL_PARKING: ("forest green", "Normal Parking Region"),
        RegionType.RESTRICTED_PARKING: ("red", "Restricted Parking Region"),
    }
    ...
    def update(msg: SimCarMsg):
        ...
        if scene_static_data.vla_extension:
            # 绘制停车区域
            for region in scene_static_data.vla_extension.regions:
                style = PARKING_REGION_STYLE_MAP.get(region.type)
                if style is None:
                    continue
                outline_color, label = style
                pos = Vector2(region.pos_x, region.pos_y)
                draw_rectangle(
                    pos,
                    region.length,
                    region.width,
