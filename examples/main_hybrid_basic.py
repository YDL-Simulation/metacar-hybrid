...
def get_delta_time(sim_car_msg) -> float | None:
...
def resolve_delta_time(sim_car_msg, elapsed_s: float) -> float:
...
def run_basic_control(
    ...
    *,
    ...
    try:
        ...
        previous_tick = clock()
        last_status = previous_tick - max(0.0, status_interval_s)
        loop = api.main_loop()
        ...
        if loop is not None:
            ...
            if callable(close):
                close()
        remove_hotkeys(keyboard_module, hotkey_handles)
